...
class YandexDirectAPI:
    ...
    @staticmethod
    def safely_execute_request(api_url: str, body: str,headers: any = None,):
        while True:
            try:
    ...
    def get_account_balance(self, token: str) -> AccountBalance:
        body = {"param": {"Action": "Get",},"method": "AccountManagement",'token': token}
        headers = {"Authorization": "Bearer " + token,"Accept-Language": "ru","processingMode": "auto"}
        body = json.dumps(body, indent=4)
        req = requests.post(self.api_url + '/live/v4/json',body,headers=headers)
    ...
    def get_account_report(self, token: str, date_from: str,goals: list[dict[str, str]] | None = None) -> AccountStatistics:
        body_raw = {"method": "get","params": {"SelectionCriteria": {"DateFrom": date_from,"DateTo": datetime.now().strftime('%Y-%m-%d')},"FieldNames": ["Clicks", "Impressions", "Cost", "Conversions"],"ReportType": "CUSTOM_REPORT","DateRangeType": "CUSTOM_DATE","IncludeVAT": 'NO',"ReportName": f'report-{str(uuid.uuid4())}',"Format": "TSV","IncludeDiscount": "NO"}}
        headers = {"Authorization": "Bearer " + token,"Accept-Language": "ru","processingMode": "auto"}
        ...
        req = YandexDirectAPI.safely_execute_request(self.api_url + '/json/v5/reports',body,headers=headers)
        account_statistics = AccountStatistics.build(req.content.decode('utf-8'),goals)
