        ...
        for alert in project.alerts.all():
            if str(alert.alert_time) == current_time:
                for account in project.yandex_direct_accounts.all():
                    account_report = yandex_direct_api.get_account_report( account.api_key,date_from,project.goals)
                    account_balance = yandex_direct_api.get_account_balance(account.api_key)
